...
} from '../utils/yearlyReviewDetailInfo';
...
export default function useYearlyReviewData(
  ...
  const { reviewResult, userRank, p2eLv } = useMemo<{
    ...
    let reviewResult = {};
    const [firstDApp, firstMetHakkaDate] =
      apiResponse.first_met_hakka.split(', ');
    const transactionAmount = apiResponse.valued;
    const transactionAmountInfo = getTransactionAmountInfo(
      apiResponse.usedDApp,
      transactionAmount,
      apiResponse.tx_pr,
    );
