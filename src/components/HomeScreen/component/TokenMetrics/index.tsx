    ...
    <Box sx={styles.tokenMetricsInfoChain} ml='2'>
      ...
      <Box sx={styles.tokenMetricsInfoContainer} mt='20px'>
        ...
        <Box sx={styles.tokenMetricsInfo} mt='1'>
          TYPE: <span sx={styles.info}>{tokenMetric.type}</span>
        </Box>
      ...
      <Box sx={styles.tokenMetricsChainList} mt='20px'>
        Get HAKKA Token on {tokenMetric.shortName}:
      </Box>
      {tokenMetric.id === 'bsc' ? (
...
function TokenMetrics() {
  ...
  const [selectedTokenMetric, setSelectedTokenMetric] = useState(
    tokenMetrics[0],
  );
  ...
  useEffect(() => {
    const currentChain = chainsInfo.find((chain) => chain.chainId === chainId);
    if (!currentChain?.id) {
      return;
    }
  ...
  const renderChain = () =>
    chainsInfo.map((chain) => (
      <Flex
        sx={
          selectedCoin === chain.id
            ? styles.chainContent_active
            : styles.chainContent
        }
          ...
          >
            <img
              sx={
                selectedCoin === chain.id
                  ? styles.imgChain
                  : styles.imgChain_gray
              }
              src={images[chain.imgChain]}
              alt=''
            />
          </Box>
          <Box
