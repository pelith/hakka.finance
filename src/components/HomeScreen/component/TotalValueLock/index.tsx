...
function TotalValueLock() {
  ...
  const renderFlagshipProducts = () =>
    products.map((item, i) => (
      <FlagshipProduct key={item.title} item={item} i={i} link={item.link} />
    ));

  const [lockedValue, setLockedValue] = useState('');
  useEffect(() => {
    fetch('https://tvl.hakka.finance/')
      .then((res) => res.text())
      .then((res) => {
        const value = Number.parseInt(res, 10).toLocaleString();
  ...
  return (
    ...
    >
      <Box>
        ...
        <Box>
          <Box sx={styles.totalValueSubText} mt='4'>
      ...
      <Box>
        <img sx={styles.totalValueImg} src={images.imageAlienGrey} alt='' />
      </Box>
    </Flex>
  );
}
