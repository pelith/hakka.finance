...
export default memo(function StakePositionTable(props: IProps) {
  ...
  const tableData: ITableData[] = useMemo(() => {
    ...
    data.forEach((raw, i) => {
      let state = HakkaVaultState.ARCHIVE;
      if (raw.unlockTime.toNumber() <= getUnixTime(Date.now())) {
        state = HakkaVaultState.REDEEM;
      }
      if (raw.hakkaAmount.gt(0)) {
        state = HakkaVaultState.STAKING;
      }
      const _tmpRaw = {
        ...raw,
        state,
        stakedHakkaStr: formatCommonNumber(raw.hakkaAmount),
        sHakkaReceivedStr: formatCommonNumber(raw.wAmount),
        index: i,
      };
      if (state) {
    ...
    archiveList.sort(createBigNumberSort('asc', 'unlockTime'));
    nonArchiveList.sort(createBigNumberSort('asc', 'unlockTime'));
  ...
  const stakedHakkaRenderer = useCallback(
    createRenderValue('HAKKA', 'stakedHakkaStr'),
    [],
  );

  const sHakkaObtainedRenderer = useCallback(
    createRenderValue('sHAKKA', 'sHakkaReceivedStr'),
    [],
  );

  const TableWrap = useCallback(
    _cond<{ tableLength: number; isMobile: boolean }, React.ReactElement>([
      [
        _matches({ tableLength: 0 }),
        () => <div sx={styles.emptySection}>No position</div>,
      ],
      [
      ...
      [
        ...
        () => (
          <Table rowKey='index' sx={styles.tableWrapper} data={tableData}>
            ...
            <Column<ITableData>
              title='HAKKA staked'
              dataIndex='hakkaAmount'
              render={stakedHakkaRenderer}
              width={180}
            />
            <Column<ITableData>
