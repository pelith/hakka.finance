...
const ScoreModal = ({ p2eLevel, performanceList, userRank }: Props) => {
  ...
  const encodedShareToTwitterUri =
    'http://twitter.com/share?' + urlStr.toString();
  const encodedShareToTelegramUri =
    'https://t.me/share/url?' + urlStr.toString();
    ...
    <Modal isOpen={scoreModalOpen} onDismiss={toggleScoreModal}>
      <div sx={styles.container}>
        ...
        <div sx={styles.shareCard} ref={eleRef}>
          <img
            src={YEARLY_REVIEW_RANK_IMAGE[userRank]}
            sx={styles.rankIconWrapper}
          />
          ...
          <div sx={styles.shareCardPerformanceContainer}>
            {performanceList.map((ele, index) => (
              <div key={index} sx={styles.shareCardPerformanceWrapper}>
                <img src={ele.icon} />
                <span style={index === 0 ? { maxWidth: '180px' } : {}}>
                  {ele.shortContent}
                </span>
              </div>
            ))}
        ...
        <div sx={styles.saveBtn}>
          <MyButton onClick={downloadImage} styleKit='green'>
            Save
          </MyButton>
        </div>
        <div sx={styles.shareSection}>
          <span>Share to</span>
          <img
            src={images.iconTelegram}
            onClick={() => {
              window.open(encodedShareToTelegramUri, '_blank').focus();
            }}
          />
          <img
            src={images.iconTwitter}
            onClick={() => {
              window.open(encodedShareToTwitterUri, '_blank').focus();
            }}
          />
        </div>
      </div>
