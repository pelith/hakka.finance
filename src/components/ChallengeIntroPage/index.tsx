...
const ChallengeIntroPage = () => {
  ...
  const isConnected = error instanceof UnsupportedChainIdError || !!account;
  const activeChain = Object.keys(_omit(ChainNameWithIcon, [ChainId.KOVAN, ChainId.RINKEBY])).map((ele) => parseInt(ele));
  const isCorrectNetwork = chainId ? activeChain.includes(chainId) : false;
