...
export const getTransactionAmountInfo = (
  usedDApp: string,
  transactionAmount: number,
  transactionPr: number,
) => {
  ...
  const transactionShortContent = usedDAppList
    .slice(0, DAPP_DISPLAY_UPPER_LIMIT)
    .join(', ');
  ...
  const transactionAmountInfo = {
    transactionAmount: {
      title:
        'Your total transaction amount and products you have used on Hakka last year:',
...
export const getGasInfo = (
  gasData: { [chainId in ChainId]: number },
  gasPr: number,
) => {
  let gasPerformance = '';
  let gasShortContent = '';
  ...
  Object.keys(gasData).map((chainId) => {
    if (gasData[chainId] > 0) {
      const targetChainInfo = chainsInfo.find(
        (chainInfo) => chainInfo.chainId === parseInt(chainId),
      );
