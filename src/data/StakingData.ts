...
export function useStakingDataV1(_chainId?: ChainId) {
  ...
  return useReadContracts({
    ...
    query: {
      enabled: isAddress(account as Address) && isAddress(STAKING_ADDRESSES[usingChainId as ChainId]),
      select(data) {
        const [stakingBalance, sHakkaBalance, votingPower, ...stakingRate] = data;
        const result = {
          stakingBalance: '',
          sHakkaBalance: '',
          votingPower: '',
        ...
        if (stakingBalance.status === 'success') {
          result.stakingBalance = formatUnits(stakingBalance.result as bigint, 18);
        }
        if (sHakkaBalance.status === 'success') {
          result.sHakkaBalance = formatUnits(sHakkaBalance.result as bigint, 18);
        }
        ...
        if (stakingRate.every((rate) => rate.status === 'success')) {
          result.stakingRate = stakingRate.map((rate) => rate.result?.toString() ?? '');
        }
        return result;
      }
    }
...
export function useStakingDataV2(_chainId?: ChainId) {
  ...
  return useReadContracts({
    ...
    query: {
      enabled: isAddress(account as Address) && isAddress(NEW_SHAKKA_ADDRESSES[usingChainId as ChainId]),
      select(data) {
        const [stakingBalance, sHakkaBalance, votingPower, ...stakingRate] = data;
        const result = {
          stakingBalance: '',
